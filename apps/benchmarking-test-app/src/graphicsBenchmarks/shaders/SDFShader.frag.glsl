...
layout(set = 1, binding = 1) uniform texture2D noiseTexture;
layout(set = 1, binding = 2) uniform texture2D uDepthTexture;
...
float cloudShape(vec3 p) {
  ...
  float density = texture(sampler2D(noiseTexture, samp), vec2(x, y)).r;
...
vec4 rayMarchClouds(vec3 ro, vec3 rd) {
  ...
  for(int i = 0; i < MAX_STEPS && d < MAX_DIST; i++) {
    ...
    float depthFromTexture = texture(sampler2D(uDepthTexture, samp), vUv).r * 10.0;
...
vec3 estimateNormal(vec3 p, vec2 torusParams) {
  ...
  return normalize(vec3(shortestDistanceToTorus(p + vec3(epsilon, 0.0, 0.0), torusParams) - shortestDistanceToTorus(p - vec3(epsilon, 0.0, 0.0), torusParams), shortestDistanceToTorus(p + vec3(0.0, epsilon, 0.0), torusParams) - shortestDistanceToTorus(p - vec3(0.0, epsilon, 0.0), torusParams), shortestDistanceToTorus(p + vec3(0.0, 0.0, epsilon), torusParams) - shortestDistanceToTorus(p - vec3(0.0, 0.0, epsilon), torusParams)));
}
...
vec3 rayMarchPhong(vec3 ro, vec3 rd) {
  ...
  for(int i = 0; i < MAX_STEPS && d < MAX_DIST; i++) {
    ...
    if(dist < MIN_DIST) {
      ...
      float depth1 = texture(sampler2D(uDepthTexture, samp), vUv).r * 0.5;
...
void main() {
  ...
  FragColor = texture(sampler2D(inputBufferTexture, samp), uv);
  if(mode == MODE_TORUS) {
    ...
    vec4 backgroundColor = texture(sampler2D(inputBufferTexture, samp), uv);
